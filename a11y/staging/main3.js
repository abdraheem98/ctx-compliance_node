...
async function runScan() {
    ...
    for (let i=0; i<scanListLen; i++) {
        ...
        await driver.get( urlsToScan[i].url );
        setContinuum();
...
async function sendAccessibilityConcernsToCTXAx( ampReportData ) {
	...
	/*
	try {
		const response = await fetch(sptIntakeUrl, 
		{
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
			body: ampReportData[0]
		});
		
		//const resultAsJson = await response.json();
		//console.log("sendAccessibilityConcernsToCTXAx() data returned: ", resultAsJson);
		console.log("sendAccessibilityConcernsToCTXAx() data returned: ", response);

	} catch(error) {

		//TODO: Integrate error notification system
		console.error(`sendAccessibilityConcernsToCTXAx(): error: ${error}` );
	}
    */

}

async function setAmpConnection() {
	console.log("setAmpConnection(): called");
	...
	try {
        ...
        status = true;
	    await ampReportingService.setActiveOrganization( ampReportData.activeOrg );  // ID of AMP organization to submit test results to

	} catch(error) {

		console.error(`setAmpConnection(): error: ${error}` );
...
async function sendAccessibilityConcernsToAMP(driver, accessibilityConcerns) {
	const ampReportingService = Continuum.AMPReportingService;
	console.log("sendAccessibilityConcernsToAMP(): called");
	
	//Build accessibility report name
	//build this into the module name
	let today = new Date();
	...
	let teamName = "Team_Identifier"; //team identifier should be added here
	let reportName = today + "_" + teamName; 

	ampReportData.pageTitle = await driver.getTitle();
	if ( !ampReportData.pageTitle || ampReportData.pageTitle == "") ampReportData.pageTitle = "NoTitleFound";
	//reportName = ampReportData.pageTitle.replace(//^[a-z0-9]+$/i/,"")

	ampReportData.url = await driver.getCurrentUrl();
	ampReportData.activeOrg = 10003; //10003 is the "Continuum Users org, 10005 is Strategic Platforms - adjust appropriate
	ampReportData.activeAsset = 38837;
	ampReportData.reportName = reportName; 
	ampReportData.reportDate = today;
	ampReportData.accessibilityConcerns = accessibilityConcerns;

	//Set report and asset data and send to AMP
	await ampReportingService.setActiveOrganization( ampReportData.activeOrg );  // ID of AMP organization to submit test results to
	await ampReportingService.setActiveAsset( ampReportData.activeAsset );  // ID of AMP asset to submit test results to
	await ampReportingService.setActiveReportByName(ampReportData.reportName);
	await ampReportingService.setActiveModuleByName( ampReportData.pageTitle, ampReportData.url );
	...
	try {
		...
		await ampReportingService.submitAccessibilityConcernsToAMP(ampReportData.accessibilityConcerns);
		ampReportData.ampReportUrl = ampReportingService.activeModule.getAMPUrl();
		ampReportData.ampSubmitState = "success";

	} catch(error) {

		//TODO: Integrate error notification system
		console.error(`sendAccessibilityConcernsToAMP(): error: ${error}` );
		ampReportData.ampSubmitState = error;

	}

	
	return ampReportData;
